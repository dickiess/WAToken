...
@interface Unrar4iOS2 (PrivateMethods)
...
@implementation Unrar4iOS2

//@synthesize filename, password;

int CALLBACK CallbackProcc(UINT msg, long UserData, long P1, long P2) {
    UInt8 **buffer;
	
	switch(msg) {
		case UCM_CHANGEVOLUME: break;
...
- (NSArray *)unrarListFiles {
    ...
    if ([self _unrarOpenFile:_filename inMode:RAR_OM_LIST_INCSPLIT withPassword:_password] == NO) {
        [self _unrarCloseFile];
        return nil;
    }
...
- (BOOL)unrarFileTo:(NSString *)path overWrite:(BOOL)overwrite {
    ...
    if ([self _unrarOpenFile:_filename inMode:RAR_OM_EXTRACT withPassword:_password ] == NO) {
        [self _unrarCloseFile];
        return NO;
    }
...
- (NSData *)extractStream:(NSString *)aFile {
	...
	RARSetCallback(_rarFile, CallbackProcc, (long) &callBackBuffer);
