...
- (id)initWithFileManager:(NSFileManager *)fileManager {
	if ( self = [super init] ) {
        ...
        self.stringEncoding = CFStringConvertEncodingToNSStringEncoding(kCFStringEncodingGB_18030_2000);//NSUTF8StringEncoding;
...
- (BOOL)CreateZipFile:(NSString *)zipFile {
    return [self CreateZipFile:zipFile append:NO];
}

- (BOOL)CreateZipFile:(NSString *)zipFile append:(BOOL)isAppend {
...
- (BOOL)CreateZipFile:(NSString *)zipFile Password:(NSString *)password append:(BOOL)isAppend {
    self.password = password;
    return [self CreateZipFile:zipFile append:isAppend];
}
...
- (BOOL)AddDataToZip:(NSData *)data fileAttributes:(NSDictionary *)attr newname:(NSString *)newname {
    ...
    if ( attr ) { fileDate = (NSDate*)[attr objectForKey:NSFileModificationDate]; }

    if ( fileDate == nil ) { fileDate = [NSDate date]; }
    
    NSCalendar *gregorianCalendar = [[NSCalendar alloc] initWithCalendarIdentifier:NSCalendarIdentifierGregorian];
    NSDateComponents *components = [gregorianCalendar components:NSCalendarUnitYear |
                                    NSCalendarUnitMonth | NSCalendarUnitDay |
                                    NSCalendarUnitHour | NSCalendarUnitMinute |
                                    NSCalendarUnitSecond fromDate:fileDate];
...
- (BOOL)UnzipFileTo:(NSString *)path overWrite:(BOOL)overwrite {
	...
	do {
        @autoreleasepool {
            ...
            if ( [strPath rangeOfCharacterFromSet:cSet].location != NSNotFound ) {
                // contains a path
