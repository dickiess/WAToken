...
- (void)addAction {
    if (self.images.count >= _formItem.maxImageCount) {
        ...
        = [[UIAlertView alloc] initWithTitle:[NSString stringWithFormat:@"最多选择%ld张附件",(long)_formItem.maxImageCount]
                                     message:nil
                                    delegate:nil
...
- (void)localPhoto {
    ...
    = [[TZImagePickerController alloc] initWithMaxImagesCount:(_formItem.maxImageCount - self.images.count)
                                                     delegate:self];
...
- (void)AuthorzationCamera {
    ...
    if (status == AVAuthorizationStatusDenied) {
        ...
        //SEL aSel = @selector(initWithFetchRequest:managedObjectContext:sectionNameKeyPath:cacheName:);
        //if ([AVCaptureDevice instancesRespondToSelector:aSel]) {
