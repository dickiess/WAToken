...
- (void)layoutSubviews {
    ...
    //重新计算textView frame
    self.textView.frame = CGRectMake(TitleWidth + 2*EdgeMarin, EdgeMarin,
                                     ScreenWidth - (TitleWidth + 3*EdgeMarin),
...
+ (CGFloat)cellHeightWithItem:(SelwynFormItem *)item {
    CGSize detailSize
    = [SelwynFormHandle getSizeWithString:item.formDetail
                                     Font:Font(TitleFont)
                                  maxSize:CGSizeMake(ScreenWidth - (TitleWidth + 3*EdgeMarin), MAXFLOAT)];
