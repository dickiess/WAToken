...
- (void)viewDidLoad {
    ...
    if ([_model.name isEqualToString:@"相机胶卷"] || [_model.name isEqualToString:@"Camera Roll"]) {
        [[TZImageManager manager] getAssetsFromFetchResult:_model.result allowPickingVideo:_tzImagePickerVc.allowPickingVideo allowPickingImage:_tzImagePickerVc.allowPickingImage completion:^(NSArray<TZAssetModel *> *models) {
            self->_photoArr = [NSMutableArray arrayWithArray:models];
            [self initSubviews];
        }];
    } else {
...
- (void)configBottomToolBar {
    ...
    _numberLable.text = [NSString stringWithFormat:@"%lu",(unsigned long)_tzImagePickerVc.selectedModels.count];
...
- (void)okButtonClick {
    ...
    for (NSInteger i = 0; i < _tzImagePickerVc.selectedModels.count; i++) {
        ...
        [[TZImageManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
            ...
            if (photo) {
                photo = [self scaleImage:photo toSize:CGSizeMake(self->_tzImagePickerVc.photoWidth, self->_tzImagePickerVc.photoWidth * photo.size.height / photo.size.width)];
            ...
            if ([self->_tzImagePickerVc.pickerDelegate respondsToSelector:@selector(imagePickerController:didFinishPickingPhotos:sourceAssets:isSelectOriginalPhoto:)]) {
                [self->_tzImagePickerVc.pickerDelegate imagePickerController:self->_tzImagePickerVc didFinishPickingPhotos:photos sourceAssets:assets isSelectOriginalPhoto:self->_isSelectOriginalPhoto];
            }
            if ([self->_tzImagePickerVc.pickerDelegate respondsToSelector:@selector(imagePickerController:didFinishPickingPhotos:sourceAssets:isSelectOriginalPhoto:infos:)]) {
                [self->_tzImagePickerVc.pickerDelegate imagePickerController:self->_tzImagePickerVc didFinishPickingPhotos:photos sourceAssets:assets isSelectOriginalPhoto:self->_isSelectOriginalPhoto infos:infoArr];
            }
            if (self->_tzImagePickerVc.didFinishPickingPhotosHandle) {
                self->_tzImagePickerVc.didFinishPickingPhotosHandle(photos,assets,self->_isSelectOriginalPhoto);
            }
            if (self->_tzImagePickerVc.didFinishPickingPhotosWithInfosHandle) {
                self->_tzImagePickerVc.didFinishPickingPhotosWithInfosHandle(photos,assets,self->_isSelectOriginalPhoto,infoArr);
            }
            [self->_tzImagePickerVc hideProgressHUD];
            [self.navigationController dismissViewControllerAnimated:YES completion:nil];
        }];
    }
}
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        ...
        if (isSelected) {
            ...
            if (weakSelf.tzImagePickerVc.selectedModels.count < weakSelf.tzImagePickerVc.maxImagesCount) {
                ...
                [weakSelf.tzImagePickerVc showAlertWithTitle:[NSString stringWithFormat:@"你最多只能选择%ld张照片",(long)weakSelf.tzImagePickerVc.maxImagesCount]];
            }
        }
...
- (void)refreshBottomToolBarStatus {
    ...
    _numberLable.text = [NSString stringWithFormat:@"%lu",(unsigned long)imagePickerVc.selectedModels.count];
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    if (error) {
        ...
        [[TZImageManager manager] getCameraRollAlbum:_tzImagePickerVc.allowPickingVideo allowPickingImage:_tzImagePickerVc.allowPickingImage completion:^(TZAlbumModel *model) {
            self->_model = model;
            [[TZImageManager manager] getAssetsFromFetchResult:self->_model.result allowPickingVideo:self->_tzImagePickerVc.allowPickingVideo allowPickingImage:self->_tzImagePickerVc.allowPickingImage completion:^(NSArray<TZAssetModel *> *models) {
                [self->_tzImagePickerVc hideProgressHUD];
            
                TZAssetModel *model = [models lastObject];
                [self->_photoArr addObject:model];
                if (self->_tzImagePickerVc.selectedModels.count < self->_tzImagePickerVc.maxImagesCount) {
                    model.isSelected = YES;
                    [self->_tzImagePickerVc.selectedModels addObject:model];
                    [self refreshBottomToolBarStatus];
                }
                [self->_collectionView reloadData];
