...
- (void)layoutSubviews {
    ...
    // 重新计算textView frame
    self.textView.frame = CGRectMake(TitleWidth + 2*EdgeMarin,
                                     EdgeMarin,
                                     ScreenWidth - (TitleWidth + 2*EdgeMarin + 30),
                                     MAX(TitleHeight, newHeight - 2*EdgeMarin));
    self.textView.textContainerInset = UIEdgeInsetsMake(3, 0, 0, 0);    // 校准textview文字位置
}

/** 动态获取cell高度 */

+ (CGFloat)cellHeightWithItem:(SelwynFormItem *)item {
    CGSize maxSize = CGSizeMake(ScreenWidth - TitleWidth - 2*EdgeMarin - 30, MAXFLOAT);
    CGSize detailSize
    = [SelwynFormHandle getSizeWithString:item.formDetail Font:Font(TitleFont) maxSize:maxSize];
