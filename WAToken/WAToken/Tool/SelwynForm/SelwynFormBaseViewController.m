...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    if (sectionItem.headerTitle) {
        UILabel *titleLabel
        = [[UILabel alloc]initWithFrame:CGRectMake(10, 0, header.frame.size.width - 10, header.frame.size.height)];
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    SelwynFormSectionItem *sectionItem = self.mutableArray[section];
    UIView *footer
    = [[UIView alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, sectionItem.footerHeight)];
    ...
    for (int sec = 0; sec < self.mutableArray.count; sec++) {
        ...
        for (int row = 0; row < sectionItem.cellItems.count; row++) {
            ...
            if (![checkItem.formError isEqualToString:@""]) {
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:checkItem.formError
                                                                message:nil
                                                               delegate:nil
