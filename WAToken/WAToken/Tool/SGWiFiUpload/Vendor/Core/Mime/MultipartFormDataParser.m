...
- (int)numberOfBytesToLeavePendingWithData:(NSData *)data length:(NSUInteger)length encoding:(NSUInteger)encoding;
...
- (int)numberOfBytesToLeavePendingWithData:(NSData *) data length:(NSUInteger)length encoding:(NSUInteger)encoding {
    ...
    int iLen = (int)length;
    int iEcode = (int)encoding;
    
	if (iEcode == contentTransferEncoding_base64) {
		char* bytes = (char*) data.bytes;
		int i;
		for( i = iLen - 1; i > 0; i++ ) {
		...
		// is multiplier of 4.
		sizeToLeavePending = (iLen - i) & ~0x11; // size to leave pending = length-i - (length-i) %4;
		return sizeToLeavePending;
	}
	
	if (iEcode == contentTransferEncoding_quotedPrintable) {
		// we don't pass more less then 3 bytes anyway.
		if( iLen <= 2 )
			return iLen;
		// check the last bytes to be start of encoded symbol.
		const char* bytes = data.bytes + iLen - 2;
		if( bytes[0] == '=' )
			return 2;
		if( bytes[1] == '=' )
			return 1;
