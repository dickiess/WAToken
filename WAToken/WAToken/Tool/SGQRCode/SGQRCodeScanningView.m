...
/** 扫描内容的 x 值 */
#define scanBorderX 0.5 * (1.0 - 0.7) * self.frame.size.width
/** 扫描内容的 Y 值 */
...
- (void)drawRect:(CGRect)rect {
    ...
    // 设置空白区
    UIBezierPath *bezierPath = [UIBezierPath bezierPathWithRect:CGRectMake(borderX + 0.5 * borderLineW,
                                                                           borderY + 0.5 *borderLineW,
                                                                           borderW - borderLineW,
                                                                           borderH - borderLineW)];
    [bezierPath fill];
    // 执行混合模式
        ...
        if (flag) {
            ...
            if (_scanningline.frame.origin.y >= - scanBorderW) {
                ...
                if (self.scanningline.frame.origin.y >= scanContent_MaxY) {
                    ...
                    [UIView animateWithDuration:self.animationTimeInterval
                                     animations:^{
                                         frame.origin.y += 2;
                                         self.scanningline.frame = frame;
                                     } completion:nil];
