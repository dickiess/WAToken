...
+ (NSString *)hmsStringWithFloat:(CGFloat)seconds{
    ...
    if (hour == 0) {
        timeString = [NSString stringWithFormat:@"%02ld:%02ld", (long)minutes, (long)second];
    }else{
        timeString = [NSString stringWithFormat:@"%02ld:%02ld:%02ld", (long)hour, (long)minutes, (long)second];
    }
