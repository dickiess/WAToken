...
#if kUseScreenShotGesture
        UIView *view = [AppDelegate shareAppDelegate].window.rootViewController.view;
        [view addObserver:self
               forKeyPath:@"transform"
                  options:NSKeyValueObservingOptionNew
                  context:szListenTabbarViewMove];
#endif
    }
    return self;
}
...
- (void)showEffectChange:(CGPoint)pt {
    if (pt.x > 0) {
        ...
        = [UIColor colorWithHue:0 saturation:0 brightness:0 alpha:-pt.x / 320.0 * 0.4 + 0.4];
        _imgView.transform
        = CGAffineTransformMakeScale(0.95 + (pt.x / 320.0 * 0.05), 0.95 + (pt.x / 320.0 * 0.05));
    }
}
...
- (void)screenShot {
    ...
    UIGraphicsBeginImageContextWithOptions(CGSizeMake([UIScreen mainScreen].bounds.size.width,
...
#if kUseScreenShotGesture
    UIView *view = [AppDelegate shareAppDelegate].window.rootViewController.view;
    [view removeObserver:self forKeyPath:@"transform" context:szListenTabbarViewMove];
#endif
}
