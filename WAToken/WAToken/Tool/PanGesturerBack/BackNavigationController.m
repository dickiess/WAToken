...
@interface BackNavigationController () <UIGestureRecognizerDelegate, UINavigationControllerDelegate>
...
- (void)configNavigationBar {
    ...
    NSDictionary *attributeDic = [NSDictionary dictionaryWithObjectsAndKeys:[UIFont systemFontOfSize:17.0],
                                  NSFontAttributeName,
                                  [UIColor whiteColor],
...
//    [UINavigationBar appearance].barTintColor = [UIColor orangeColor];
    [UINavigationBar appearance].barTintColor = [UIColor colorWithWhite:0.5f alpha:0.8f];
    [UINavigationBar appearance].tintColor = [UIColor whiteColor];
}

- (BOOL)gestureRecognizerShouldBegin:(UIPanGestureRecognizer *)gestureRecognizer {
    if (gestureRecognizer.view == self.view) {
        PanBackViewController *topView = (PanBackViewController *)self.topViewController;
...
#if kUseScreenShotGesture
    AppDelegate *appdelegate = (AppDelegate *)[UIApplication sharedApplication].delegate;
    CGSize size = CGSizeMake(appdelegate.window.frame.size.width, appdelegate.window.frame.size.height);
